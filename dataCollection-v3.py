...
def readData(filename, port, duration):
    ...
    try:
        ...
        while time.time() < runtime:
            inputData_raw.append(list(arduino.read(6)))
        ...
        print("[INFO] Collected: " + str(count) + " data in " + str(round((stop - start), 4)) + " second(s)")
...
try:
    print("[INFO] Program started\n[INFO] Port: " + sys.argv[1] + " Duration: " + sys.argv[2] + " second(s)")
    readData(csv_file_string, sys.argv[1],sys.argv[2])
    
except Exception as e:
    print("[Error-argv] Format dataCollection-v3.py [COM] [Duration]" + str(e))
    sys.exit(1)